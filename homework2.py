...
class Student:
    ...
    def __init__ (self, name, birthdate, email):
        ...
        if birthdate>=date.today(): raise ValueError
        self.email = email
...
class Course:
    ...
    def __init__ (self, title, start_date, end_date):
        self.title = title
        if title == "": raise ValueError
        self.start_date = start_date
        self.end_date = end_date
        if start_date > end_date: raise ValueError
    def enroll (self):
        if self.end_date < date.today():
            raise Exception ("CourseFinished")
        else: print ("Registered")
        if self.title == 'Python/Django for beginners':
            raise Exception ("AlreadyEnrolled")
    def num_enrolled (self):
        Course.num_students=Course.num_students + 1
        return Course.num_students
